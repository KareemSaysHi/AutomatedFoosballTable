...
class OnnxHelper():
    ...
    def setInputs(self, ballPos, rodPosArray, rodRotArray):
        ...
        #print(ballPos)
        #print("this is rodPos")
        #print(rodPosArray)
        #print("this is rodRot")
        #print(rodRotArray)
        try:
    ...
    def runOnnxThrees(self, ballPos, rodPosArray, rodRotArray, heuristic = False, manufacturedOutput = []):
        ...
        linoff, linmid, lindef, rotoff, rotmid, rotdef = self.rawOutputToVelThrees(rodPosArray, rodRotArray, outputs)
    ...
    def rawOutputToVel(self, rodPosArray, rodRotArray, outputArray): #currently only for one rod
        ...
        if (self.rotaryInput >= 2):
            self.rotaryInput = 2
            self.foosAngularVelocity = 1200
        elif (self.rotaryInput == 1):
            self.foosAngularVelocity = 600
        ...
        if newRodRotArray[1] > self.maxRotation:
            newRodRotArray[1] = self.maxRotation
            self.foosAngularVelocity = 0
            self.rotaryInput = 0
        elif newRodRotArray[1] < -1*self.maxRotation:
    ...
    def rawOutputToVelThrees(self, rodPosArray, rodRotArray, outputArray): #this one works for three
        ...
        for i in range (0, 3):
            ...
            elif (self.linearInputs[i] == 2):
                self.linearVelocities[i] = 8
            elif (self.linearInputs[i] == 1):
                self.linearVelocities[i] = 2
            elif (self.linearInputs[i] == 0):
        ...
        return [*self.linearVelocities, *self.angularVelocities]
